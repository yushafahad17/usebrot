...
from .tools import UputtChanger
    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            iamuput_reg = sudo_reg = re.compile(pattern)
        else:
            iamuput_ = "\\" + var.CMD_HANDLER
            sudo_ = "\\" + var.SUDO_HANDLER
            iamuput_reg = re.compile(iamuput_ + pattern)
            sudo_reg = re.compile(sudo_ + pattern)
            if command is not None:
                cmd1 = iamuput_ + command
                cmd2 = sudo_ + command
            else:
                cmd1 = (
                    (iamuput_ +
                ...
                if not disable_edited:
                    Uputt.add_event_handler(
                        ...
                        events.MessageEdited(
                            **args,
                            from_users=UputtChanger(sudoer()),
                            pattern=sudo_reg
                        ),
                    )
                Uputt.add_event_handler(
...
def chataction(**args):
    def decorator(func):
        if Uputt:
            Uputt.add_event_handler(func, events.ChatAction(**args))
        return func
