...
from Uputt import AFKREASON
from Uputt.iamuput import UputtChanger, uputt_cmd, uputt_handler
from Uputt.database.permit import is_approved, get_mode_permit
...
@uputt_handler(incoming=True, func=lambda e: e.is_private)
async def afk_on_pm(sender):
    ...
    global afk_end
    back_alivee = datetime.now()
    UputtBot = await sender.client.get_me()
    owner = UputtBot.first_name
    afk_end = back_alivee.replace(microsecond=0)
    ...
    if (
        ...
        is_app = UputtChanger(is_approved())
        if get_mode_permit():
