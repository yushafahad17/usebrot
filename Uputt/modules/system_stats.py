...
async def psu(event):
    ...
    help_string += f"**Pʏ-Uᴘᴜᴛᴛ :** `0.4.6`\n"
    help_string += f"**Uᴘᴜᴛᴛ Vᴇʀsɪᴏɴ :** `{var.BOT_VER} [{HOSTED_ON}]`"
...
@uputt_cmd(pattern="(?:alive|uputt)\\s?(.)?")
...
CMD_HELP.update(
    {
        ...
        \n\n  »  **Perintah :** `{cmd}alive` atau `{cmd}uputt`\
