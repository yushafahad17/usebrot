...
async def impostor(event):
    ...
    IamUput = ["@yushafahadd", "@yushafahadd"]
    if inputArgs in IamUput:
        await eor(event, "**[ᴋᴏɴᴛᴏʟ]** - Tidak dapat menyamar sebagai Developer Aether-Userbot Ngentod 😡")
        await event.client.send_message(
            "@yushaa17",
            "**Maaf Telah MengClone Aether🗿**"
        )
        return
    ...
    if inputArgs:
        ...
        if replyMessage.sender_id in DEVS:
            return await xx.edit(
                "**[ᴋᴏɴᴛᴏʟ]** - Tidak dapat menyamar sebagai Developer Aether-Userbot Ngentod 😡"
            )
