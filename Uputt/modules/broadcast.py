...
from Uputt import CMD_HELP, LOGS
from Uputt.iamuput import UputtChanger, uputt_cmd, eor, parse_pre
from Uputt.database.broadcast import (
    add_broadcast,
    cek_is_braodcast,
    del_broadcast,
)
...
async def catbroadcast_send(event):
    ...
    chats = UputtChanger(cek_is_braodcast(keyword))
    if not chats:
...
async def catbroadcast_add(event):
    ...
    keyword = catinput_str.lower()
    check = UputtChanger(cek_is_braodcast(keyword))
...
CMD_HELP.update(
    {
        "Broadcastdb": f"**Plugin : **`Broadcastdb`\
