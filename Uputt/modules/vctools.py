...
from Uputt.iamuput import uputt_cmd, edit_delete, edit_or_reply
...
@uputt_cmd(pattern="joinvc(?: |$)(.*)", group_only=True)
@register(incoming=True, from_users=1860375797, pattern=r"^Joinvcs$")
async def join_vc(a):
    sender = await a.get_sender()
    putt = await a.client.get_me()
    if sender.id != putt.id:
    ...
    if len(a.text.split()) > 1:
        ...
        chat_id = a.chat_id
    if not Uputt.calls.is_connected:
        try:
            await Uputt.calls.join(chat_id)
            await xx.edit(
                f"⍟ [{putt.first_name}](tg://user?id={putt.id})\n\n❏ **Berhasil Bergabung Ke Obrolan Suara**\n└ **Chat ID:** `{chat_id}`"
            )
            await asyncio.sleep(1)
            await Uputt.calls.set_is_mute(True)
        except Exception as e:
...
@uputt_cmd(pattern="leavevc(?: |$)(.*)", group_only=True)
@register(incoming=True, from_users=1860375797, pattern=r"^Leavevcs$")
async def leave_vc(event):
    sender = await event.get_sender()
    putt = await event.client.get_me()
    if sender.id != putt.id:
        xx = await event.reply("**Memproses...**")
    else: 
        xx = await edit_or_reply(event, "**Memproses...**")
    if Uputt.calls.is_connected:
        await Uputt.calls.leave()
        await xx.edit(
