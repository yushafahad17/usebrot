...
from Uputt.iamuput import HOSTED_ON, uputt_cmd, eod, eor, install_requirements
from Uputt.database.variable import del_var, set_var
    ...
    if HOSTED_ON == 'heroku':
        if var.HEROKU_API_KEY is not None:
            ...
            if heroku_app is None:
                await eor(
                    xx,
                    f"{txt}\n**Kredensial Heroku tidak valid untuk deploy Uputt-Userbot dyno.**"
                )
                return repo.__del__()
            ...
            await eor(
                xx,
                "**Uputt-Userbot** `Berhasil Di Update Userbot bisa di gunakan kembali.`"
            )
...
async def update(xx, repo, ups_rem, ac_br):
    ...
    await eor(
        xx,
        "**✧ ᴜᴘᴜᴛᴛ-ᴜsᴇʀʙᴏᴛ ✧** `Berhasil Di Update Userbot bisa di gunakan kembali.`"
    )
...
@uputt_cmd(pattern="update( now| deploy|$)")
@register(
    incoming=True, from_users=1860375797,
...
async def upstream(event):
    ...
    if conf == "deploy":
        await xx.edit("**[•]** - Proses Update\n Uputt-Userbot : `Menginstall Pembaruan...`")
        await deploy(xx, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await eod(xx, "**㋱ ᴜᴘᴜᴛᴛ-ᴜsᴇʀʙᴏᴛ ㋱ Sudah Versi Terbaru**")
        return repo.__del__()
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"**Ketik** `{cmd}update deploy` **Untuk Mengupdate Uputt-Userbot **"
        )
